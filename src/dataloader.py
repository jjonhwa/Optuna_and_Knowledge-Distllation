...
def create_dataloader(
    config: Dict[str, Any], subset_ratio: float = 0.0,
...
def get_dataset(
    ...
    # Only some of the training data is used to create subsets.
    if subset_ratio:
        n = int(len(train_dataset) * subset_ratio)
        train_dataset = torch.utils.data.Subset(
            train_dataset, np.random.choice(len(train_dataset), n, replace=False)
        )
